...
- 同一时间只显示一个会话（Claude Code 面板式），避免标签堆叠挤压终端
- 工具栏 `☰`：下拉会话列表（当前会话高亮；行内 `×` 关闭；退出会话点击即重启）；底部「↺ 恢复历史会话 (omp -r)」
- 工具栏 `＋`：在当前工作区根目录新起一个 omp 会话并切换为活动
- 工具栏 `✕`：退出（杀掉）当前会话进程
- 无会话时显示欢迎态：居中 π logo + 「新建会话 / 恢复历史会话」按钮 + 操作提示
- 活动会话进程退出后显示浮层：「重新开始 / 查看输出」，并提示会话记录在磁盘可恢复
